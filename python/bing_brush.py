...
class BingBrush:
    ...
    def parse_cookie(self, cookie_string):
        ...
        elif cookie_string.startswith('"') and cookie_string.endswith('"'):
            cookie_string = cookie_string[1:-1]
    ...
    def process(self, prompt, out_folder,retry_count=0):
        ...
        if redirect_url is None:
            ...
            if redirect_url is None:
                if retry_count >= 3:  # if already tried 3 times
                    print(
                        "==> Error occurs, please submit an issue at https://github.com/vra/bing_brush, I will fix it as soon as possible."
                    )
                    return -1
                else:  # if haven't tried 3 times yet
                    return self.process(prompt, out_folder, retry_count=retry_count+1)  # retry with increased retry_count
        ...
        for url in img_urls:
            if not url.endswith('.svg'):
                count+=1
                self.write_image(url, out_folder, count)
        print(f"==> Images are saved to {out_folder}")

    def write_image(self, url, out_folder,count):
